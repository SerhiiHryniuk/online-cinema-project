...
async def test_add_favorite(db_session):
    ...
    favorite = await add_favorite(db_session, user.id, inception.id)
    await db_session.commit()
...
async def test_remove_favorite(db_session):
    ...
    await add_favorite(db_session, user.id, inception.id)
    await db_session.commit()

    removed = await remove_favorite(db_session, user.id, inception.id)
    await db_session.commit()

    assert removed is True

    found = await get_favorite(db_session, user.id, inception.id)
...
async def test_favorites_list_only_users_favorites(db_session):
    ...
    items, total = await get_favorite_movies_page(
        db_session, user.id, 1, 10, params
    )
...
async def test_favorites_list_with_search(db_session):
    ...
    await add_favorite(db_session, user.id, inception.id)
    await add_favorite(db_session, user.id, matrix.id)
    await db_session.commit()
