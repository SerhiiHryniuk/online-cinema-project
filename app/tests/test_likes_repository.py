...
async def test_set_like_creates_reaction(db_session):
    ...
    like = await repo.set_like(user.id, movie.id, LikeType.LIKE)
    await db_session.commit()
...
async def test_set_like_updates_existing(db_session):
    ...
    first = await repo.set_like(user.id, movie.id, LikeType.LIKE)
    await db_session.commit()
    first_id = first.id

    updated = await repo.set_like(user.id, movie.id, LikeType.DISLIKE)
    await db_session.commit()
...
async def test_remove_like(db_session):
    ...
    await repo.set_like(user.id, movie.id, LikeType.LIKE)
    await db_session.commit()

    removed = await repo.remove_like(user.id, movie.id)
    await db_session.commit()

    assert removed is True

    like = await repo.get_user_like(user.id, movie.id)
    assert like is None
...
async def test_count_movie_likes(db_session):
    ...
    await repo.set_like(user.id, movie.id, LikeType.DISLIKE)
    await db_session.commit()

    likes, dislikes = await repo.count_movie_likes(movie.id)
